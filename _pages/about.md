---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

---

<span class="anchor" id="about-me"></span>

<p class="intro-quote">
<i class="fas fa-quote-left fa-2x fa-pull-left fa-border" aria-hidden="true"></i>
Linshuai Zhang is an Associate Professor and Master's Supervisor at Chengdu University of Traditional Chinese Medicine. He received his Ph.D. in Intelligent Mechanical Systems Engineering from Kagawa University, Japan, and completed postdoctoral research at Kagawa University. His research focuses on vascular interventional surgical robots, acupuncture robots, haptic feedback, intelligent sensing, and interactive control for medical robotic systems.
<i class="fas fa-quote-right fa-2x fa-pull-right fa-border" aria-hidden="true"></i>
</p>

<ul class="quick-facts">
  <li><i class="fas fa-university"></i> Associate Professor, Chengdu University of Traditional Chinese Medicine</li>
  <li><i class="fas fa-map-marker-alt"></i> Chengdu, Sichuan, China</li>
  <li><i class="fas fa-envelope"></i> zhanglinshuai@cdutcm.edu.cn</li>
  <li><i class="fas fa-phone"></i> 18086848801</li>
  <li><i class="fas fa-file-alt"></i> 40+ publications, 20+ patents/software copyrights, and multiple funded projects</li>
</ul>

<span class="anchor" id="news"></span>

# <i class="fas fa-rss"></i> News

<ul class="news-box">
  <li><span class="news-date">[2025]</span>: <span class="news-mark">🎉</span> A journal paper on a magnetorheological-fluid haptic interface for endovascular catheterization was published in <em>Sensors</em>.</li>
  <li><span class="news-date">[2025]</span>: <span class="news-mark">🎉</span> A journal paper on an angle-sensor-integrated upper-limb exoskeleton system was published in <em>Sensors</em>.</li>
  <li><span class="news-date">[2025]</span>: <span class="news-mark">🎉</span> A robotic system with haptic feedback for precision cardiac ablation was published in <em>Scientific Reports</em>.</li>
  <li><span class="news-date">[2025]</span>: <span class="news-mark">🎉</span> Dr. Zhang started a technology-transfer project on intelligent moxibustion physiotherapy robots.</li>
  <li><span class="news-date">[2024]</span>: <span class="news-mark">🎉</span> Dr. Zhang was recognized as an Outstanding Member of the Sichuan Artificial Intelligence Society.</li>
  <li><span class="news-date">[2024]</span>: <span class="news-mark">🎉</span> Dr. Zhang was selected as a Chengdu D-class talent and a Jinniu District D1-class talent.</li>
  <li><span class="news-date">[2024]</span>: <span class="news-mark">🎉</span> Multiple papers were presented at IEEE ICMA and IEEE ROBIO on vascular intervention, acupuncture robots, and force sensing.</li>
  <li><span class="news-date">[2023]</span>: <span class="news-mark">🎉</span> A paper on spherical underwater robot cooperation was published in <em>IEEE/ASME Transactions on Mechatronics</em>.</li>
  <li><span class="news-date">[2020]</span>: <span class="news-mark">🎉</span> Dr. Zhang was selected as a high-level overseas returnee talent of Sichuan Province.</li>
  <li><span class="news-date">[2018]</span>: <span class="news-mark">🎉</span> Dr. Zhang received the Best Conference Paper Nomination Award at IEEE ICIA 2018.</li>
</ul>

<span class="anchor" id="research"></span>

# <i class="fas fa-brain fa-spin"></i> Research Interests

- Vascular interventional surgical robots
- Acupuncture robots and intelligent traditional Chinese medicine equipment
- Haptic feedback and force/tactile sensing
- Soft actuators, sensors, and catheter distal force control
- Intelligent sensing, multi-sensor fusion, and interactive control
- Medical rehabilitation robots and upper-limb exoskeleton systems

<span class="anchor" id="projects"></span>

# <i class="fas fa-microscope"></i> Funded Research Projects

### Principal Investigator

<ul class="compact-list">
  <li><strong>Key technologies and application demonstration of intelligent moxibustion physiotherapy robots for industrialization</strong>, Sichuan Administration of Traditional Chinese Medicine, 2025-2027.</li>
  <li><strong>Dynamic precise force control of a new distal ablation catheter structure based on soft actuation and sensing</strong>, National Natural Science Foundation of China Youth Program, 2023-2025.</li>
  <li><strong>Compliant contact interaction control of a distal ablation catheter for interventional robots in dynamic cardiac chambers</strong>, Sichuan Science and Technology Program, 2023-2024.</li>
  <li><strong>GNSS/IMU/UWB-based seamless high-precision positioning for unmanned agricultural machinery</strong>, Chengdu Science and Technology Bureau, 2022-2023.</li>
  <li><strong>Multi-sensor fusion localization for autonomous unmanned systems</strong>, Open Fund of Sichuan Engineering Laboratory for Intelligent Perception and Control of Unmanned Systems, 2022-2023.</li>
  <li><strong>Safety warning mechanism for vascular interventional surgical robots based on haptic feedback</strong>, Talent Introduction Research Start-up Project, Chengdu University of Information Technology, 2021-2023.</li>
</ul>

### Key Participant

<ul class="compact-list">
  <li>Key technologies for human back acupoint recognition and localization based on deep learning, Sichuan International Science and Technology Innovation Cooperation Program, 2024-2025.</li>
  <li>Key technologies for intelligent acupuncture robots in traditional Chinese medicine, Sichuan Administration of Traditional Chinese Medicine, 2024-2026.</li>
  <li>Campus unmanned shuttle vehicle key technologies, Sichuan International Science and Technology Cooperation Program, 2021-2023.</li>
  <li>High-end real-time cardiovascular and cerebrovascular interventional assistance robotic system, National High Technology Research and Development Program of China, 2015-2020.</li>
</ul>

<span class="anchor" id="publications"></span>

# <i class="fas fa-book-open"></i> Publications

### Selected Journal Papers

<ol class="publication-list">
  <li><strong>Linshuai Zhang</strong>, Siyu Huang, Jinshan Zuo, Shuoxin Gu, Lin Xu, Yujie Zhang, and Tao Jiang. An innovative master haptic interface employing magnetorheological fluids for endovascular catheterization. <em>Sensors</em>, 25(24):7450, 2025.</li>
  <li><strong>Linshuai Zhang</strong>, Xin Tian, Yaqi Fan, Tao Jiang, Shuoxin Gu, and Lin Xu. Development and efficacy assessment of an angle sensor-integrated upper limb exoskeleton system for autonomous rehabilitation training. <em>Sensors</em>, 25(13):3984, 2025.</li>
  <li><strong>Linshuai Zhang</strong>, Jinshan Zuo, Ke Wang, et al. An advanced robotic system incorporating haptic feedback for precision cardiac ablation procedures. <em>Scientific Reports</em>, 15(1):6839, 2025.</li>
  <li>Zheng Yu, Yusheng Tang, <strong>Linshuai Zhang*</strong>, et al. Towards the development of a novel flexible sensor for simultaneous measurement of acupoint resistance and temperature in human subjects. <em>IEEE Access</em>, 13:28766-28776, 2025.</li>
  <li><strong>Linshuai Zhang</strong>, Qian Wang, Shuoxin Gu, Tao Jiang, et al. Tightly-coupled SLAM integrating LiDAR and INS for unmanned vehicle navigation in campus environments. <em>IEEE Access</em>, 12:26441-26456, 2024.</li>
  <li>Zheng Yu, Shengyan Zhang, <strong>Linshuai Zhang*</strong>, et al. Design and performance evaluation of a home-based automatic acupoint identification and treatment system. <em>IEEE Access</em>, 12:25491-25500, 2024.</li>
  <li>Shuoxin Gu, <strong>Linshuai Zhang*</strong>, Shuxiang Guo, et al. Communication and cooperation for spherical underwater robots by using acoustic transmission. <em>IEEE/ASME Transactions on Mechatronics</em>, 28(1):292-301, 2023.</li>
  <li><strong>Linshuai Zhang</strong>, Shuoxin Gu, Shuxiang Guo, and Takashi Tamiya. A magnetorheological fluids-based robot-assisted catheter/guidewire surgery system for endovascular catheterization. <em>Micromachines</em>, 12(6):640, 2021.</li>
  <li><strong>Linshuai Zhang</strong>, Shuxiang Guo, Huadong Yu, Yu Song, Takashi Tamiya, Hideyuki Hirata, and Hidenori Ishihara. Design and performance evaluation of collision protection-based safety operation for a haptic robot-assisted catheter operating system. <em>Biomedical Microdevices</em>, 20(2):22, 2018.</li>
  <li><strong>Linshuai Zhang</strong>, Shuxiang Guo, Huadong Yu, and Yu Song. Performance evaluation of a strain-gauge force sensor for a haptic robot-assisted catheter operating system. <em>Microsystem Technologies</em>, 23(10):5041-5050, 2017.</li>
  <li>Peng Shi, Shuxiang Guo, <strong>Linshuai Zhang</strong>, et al. Design and evaluation of a haptic robot-assisted catheter operating system with collision protection function. <em>IEEE Sensors Journal</em>, 21(18):20807-20816, 2021.</li>
  <li>Shuxiang Guo, Yu Song, Xuanchun Yin, <strong>Linshuai Zhang</strong>, et al. A novel robot-assisted endovascular catheterization system with haptic force feedback. <em>IEEE Transactions on Robotics</em>, 35(3):685-696, 2019.</li>
</ol>

### Selected Conference Papers

<ol class="publication-list">
  <li>Kaixiang Zheng, Yuyang Luo, Jinshan Zuo, <strong>Linshuai Zhang*</strong>, Tao Jiang, and Dai Qiu. Design and implementation of a PWM-controlled controllable current source for robotic catheter force feedback applications. <em>IEEE ICMA</em>, pp.977-982, 2025.</li>
  <li>Xin Tian, Zhihang Yuan, <strong>Linshuai Zhang*</strong>, Tao Jiang, et al. Development and validation of a real-time force/torque sensing system for precision acupuncture robots. <em>IEEE ICMA</em>, pp.964-969, 2025.</li>
  <li>Zhihang Yuan, <strong>Linshuai Zhang*</strong>, Tao Jiang, Shuoxin Gu, Lin Xu, Yujie Zhang, Peng Zeng, and Marcin Grzegorzek. A force control method of medical robot based on impedance control. <em>IEEE ROBIO</em>, 2024.</li>
  <li>Jinshan Zuo, <strong>Linshuai Zhang*</strong>, Xinlin Hao, Yang Yang, Xinyu Zhang, and Tao Jiang. A novel vascular interventional robot for cardiac ablation surgery. <em>IEEE ICMA</em>, pp.245-250, 2024.</li>
  <li><strong>Linshuai Zhang</strong>, Yuyang Luo, Xin Tian, Shuoxin Gu, and Tao Jiang. A three-dimensional catheter distal force sensor based on fiber Bragg grating for cardiac ablation. <em>IEEE ICMA</em>, pp.519-524, 2024.</li>
  <li>Zhihang Yuan, <strong>Linshuai Zhang*</strong>, Tao Jiang, et al. Design and analysis of a novel intelligent acupuncture robot. <em>IEEE ICMA</em>, pp.765-769, 2024.</li>
  <li>Zhihang Yuan, <strong>Linshuai Zhang*</strong>, Tao Jiang, et al. A novel acupuncture needle based on FBG for quantitative evaluation of expert manipulations. <em>IEEE ICMA</em>, pp.1466-1471, 2023.</li>
  <li><strong>Linshuai Zhang</strong>, Jingjing Fan, Shuoxin Gu, et al. A novel master haptic interface based on MR-fluids for endovascular catheterization. <em>IEEE ICMA</em>, pp.1269-1274, 2022.</li>
  <li><strong>Linshuai Zhang</strong>, Shuxiang Guo, Shuoxin Gu, et al. An MR fluids-based master haptic interface with adjustable protection threshold for endovascular catheterization. <em>IEEE ROBIO</em>, pp.953-958, 2019.</li>
  <li><strong>Linshuai Zhang</strong>, Shuxiang Guo, Huadong Yu, Yu Song, and Dapeng Song. Adaptive clamping mechanism-based novel slave manipulator for endovascular catheterization. <em>IEEE ICMA</em>, pp.2358-2363, 2018.</li>
</ol>

<span class="anchor" id="patents"></span>

# <i class="fas fa-certificate"></i> Patents and Software Copyrights

<ul class="compact-list">
  <li>Interactive software for a cardiac ablation catheter robot, software copyright, 2024.</li>
  <li>Control method for chamber pressures in soft actuators, Chinese invention patent, granted, 2024.</li>
  <li>Auxiliary subsystem for the slave side of a vascular interventional robot for cardiac ablation surgery, utility model patent, granted, 2024.</li>
  <li>Sensor needle based on micro-force sensing technology and biological principles, Chinese invention patent, granted, 2023.</li>
  <li>Master-side haptic interaction device for vascular interventional surgical robots, Chinese invention patent, granted, 2023.</li>
  <li>Multi-scenario seamless localization method for unmanned equipment based on UWB, IMU, and GNSS fusion, Chinese invention patent, granted, 2023.</li>
  <li>Wearable upper-limb exoskeleton robot, utility model patent, granted, 2023.</li>
  <li>Master-side haptic feedback device for master-slave vascular interventional surgical robots, utility model patent, granted, 2022.</li>
  <li>Method for localization of autonomous mobile systems based on dynamic loading of point cloud maps, Luxembourg patent, 2022.</li>
</ul>

<span class="anchor" id="awards"></span>

# <i class="fas fa-trophy"></i> Honors and Awards

- *2024* Chengdu D-class Talent
- *2024* Jinniu District D1-class Outstanding Talent, Chengdu
- *2024* Outstanding Member of the Sichuan Artificial Intelligence Society
- *2022* Second Prize, Achievement Award of the China Invention and Entrepreneurship Award
- *2022* First Prize, Sichuan SME Innovation and Entrepreneurship Competition, "Green, Low-Carbon and New Materials" Track
- *2022* Excellence Award, Sichuan Final of the China SME Innovation and Entrepreneurship Competition
- *2020* High-level overseas returnee talent of Sichuan Province
- *2018* Kagawa University Global Human Resource Development Special Fund
- *2018* Best Conference Paper Nomination Award, IEEE ICIA 2018
- *2017* President's Commendation Award, Kagawa University, Japan
- *2017* Special Scholarship, Kuraoka Scholarship Foundation, Japan
- *2016* Outstanding Master's Thesis of Jilin Province

<span class="anchor" id="education"></span>

# <i class="fas fa-user-graduate"></i> Education

<div class="timeline-list" markdown="1">
- **2016.04 - 2019.03**, Kagawa University, Japan, Ph.D. in Intelligent Mechanical Systems Engineering
- **2015.09 - 2019.06**, Changchun University of Science and Technology, Ph.D. study in Mechanical Engineering
- **2012.09 - 2015.04**, Changchun University of Science and Technology, M.Eng. in Mechanical Manufacturing and Automation
- **2007.09 - 2011.06**, Changchun University of Science and Technology, B.Eng. in Mechanical Design, Manufacturing and Automation
</div>

<span class="anchor" id="experience"></span>

# <i class="fas fa-briefcase"></i> Work Experiences

<div class="timeline-list" markdown="1">
- **2022.12 - Present**, Chengdu University of Traditional Chinese Medicine, Distinguished Associate Professor
- **2019.10 - 2022.11**, Chengdu University of Information Technology, Lecturer
- **2019.04 - 2020.03**, Kagawa University, Japan, Collaborative Researcher / Postdoctoral Researcher
- **2016.10 - 2019.02**, Kagawa University, Japan, Teaching Assistant
- **2016.04 - 2019.03**, Kagawa University, Japan, Research Assistant
</div>

<span class="anchor" id="teaching"></span>

# <i class="fas fa-chalkboard-teacher"></i> Teaching and Student Mentoring

### Teaching Projects and Textbooks

<ul class="compact-list">
  <li>Innovative experimental project construction for <em>Medical Robotics</em> oriented toward outstanding innovative talent cultivation in the context of new medicine, university-level key teaching reform project, PI, 2023.</li>
  <li>Co-editor, <em>Embedded System Development and Practice: Based on STM32F10x Series</em>, 2nd edition, Beihang University Press, 2019.</li>
  <li>Editorial board member, <em>Fundamentals of Medical Electronics</em>, Nanjing University Press, 2024.</li>
  <li>Teaching research paper on blended teaching methods for Omron PLC courses, 2021.</li>
</ul>

### Student Projects and Competitions

<ul class="compact-list">
  <li>Supervised national-level undergraduate innovation projects on haptic sensing and feedback systems for vascular interventional surgical robots, wearable upper-limb exoskeleton rehabilitation, and MR-fluid-based haptic interaction systems.</li>
  <li>Supervised a provincial-level undergraduate innovation project on a home-based upper-limb exoskeleton self-rehabilitation training system.</li>
  <li>Guided student teams to win awards in intelligent robot creative competitions, "Internet+" innovation and entrepreneurship competitions, biomedical engineering innovation design competitions, and green manufacturing competitions.</li>
  <li>Teaching awards include university-level higher education teaching achievement awards and multimedia courseware competition award.</li>
</ul>

<span class="anchor" id="services"></span>

# <i class="fas fa-atom fa-spin"></i> Academic Services

### Professional Appointments

- Deputy Secretary-General, Sichuan Artificial Intelligence Society
- Council Member, Sichuan Artificial Intelligence Society
- Council Member, Artificial Intelligence Branch of China Association of Chinese Medicine Information
- Member, Youth Working Committee of Sichuan Artificial Intelligence Society
- Member, Traditional Chinese Medicine Information Committee of Sichuan Health Information Society
- Council Member, Shuangliu District Association of Overseas Returned Scholars, Chengdu
- Distinguished Expert of Wangcang County, Sichuan Province
- Peer Review Expert, National Natural Science Foundation of China
- Session Chair, IEEE ICMA

### <i class="fas fa-clipboard-check heading-icon"></i> Reviewer Services

- IEEE Transactions on Robotics
- IEEE Transactions on Medical Robotics and Bionics
- IEEE Sensors Journal
- IEEE Access
- Applied Surface Science
- Applied Sciences
- International Journal of Mechatronics and Automation
- Journal of Personalized Medicine
- IEEE ICMA and IEEE ROBIO
